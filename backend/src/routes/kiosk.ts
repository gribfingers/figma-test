 ...
 * number alone. Only exact matches, and only a single passenger record —
 * unlike the agent search this never substring-matches or returns other
 * passengers on the same PNR.
 */
kioskRouter.get("/lookup", (req, res) => {
  ...
  if (!row) return res.status(404).json({ error: "Booking not found — check your reference and last name" });

  const extra = readExtra(row as Passenger);
  res.json({
    passenger: serializePassenger(row as Passenger),
    flight: flightLabel(row),
    bagTags: extra.bagTags ?? [],
    bagDroppedAt: extra.bagDroppedAt ?? null,
  });
});
...
/** Bag-drop station's own lookup — by a printed tag number, or the same pnr+surname pair as check-in (in case the passenger goes straight there without the physical tag in hand, e.g. re-print scenarios). */
kioskRouter.get("/bag-drop/lookup", (req, res) => {
  ...
  if (tag) {
    rows = db
      ...
      .all() as any[];
    rows = rows.filter((r) => (readExtra(r as Passenger).bagTags ?? []).includes(tag));
  } else if (pnr && surname) {
    rows = db
      .prepare(
        `SELECT p.*, f.flight_number, f.carrier_code, f.origin, f.destination, f.std, f.status as flight_status
      ...
      .all(pnr, surname) as any[];
  } else {
    return res.status(400).json({ error: "Provide tag, or pnr+surname" });
  }

  const row = rows[0];
  if (!row) return res.status(404).json({ error: "No matching checked-in booking with bags found" });
...
/** Confirms the bag(s) were physically placed on the belt. Idempotent — re-confirming an already-dropped bag just returns the existing state rather than erroring, since a kiosk retry shouldn't dead-end the passenger. */
kioskRouter.post("/:passengerId/bag-drop", (req, res) => {
  ...
  if (!extra.bagDroppedAt) {
    ...
    // stable per passenger rather than random on every request.
    const desk = String(1 + (passenger.id % 20));
