...
const OTHER_SEAT_ATTRS = [
  "noRecline", "stretcher", "wheelchair", "animal", "child", "infant", "inst", "transit", "fixedArmrest", "legroom",
] as const;
...
/**
 * Deletes the UTC calendar day `date`'s auto-generated flights (cascading to
 * their seats/passengers) and rebuilds them from scratch with the generator's
 * current logic. Unlike generateDailySchedule, this always regenerates — for
 * when an admin wants today's demo flights to reflect a generator change
 * (new seat attributes, icon fixes, etc.) without waiting for the next UTC
 * midnight. Curated seed.ts flights (no dailyAutoGenerated marker) are untouched.
 */
export function regenerateTodaySchedule(date: Date): { flights: number; passengers: number } {
  const start = dayStartUtc(date);
  const end = new Date(start.getTime() + 24 * 60 * 60 * 1000);
  db.prepare(`DELETE FROM flights WHERE std >= ? AND std < ? AND extra LIKE ?`).run(
    start.toISOString(),
    end.toISOString(),
    `%"${AUTO_GENERATED_MARKER}":true%`
  );
  return generateDailySchedule(date);
}
